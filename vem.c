...
double mtrace (gsl_matrix *m)
{
    ...
    if (m->size1 != m->size2)
    {
        printf("mtrace: matrix is not square!\n");
        exit(0xE);
    }
    for (i=0; i<m->size1; i++)
        r *= mget(m,i,i);
...
double determinante (gsl_matrix *mtx)
{
    ...
    {
        printf("determinate: matrix is not square!\n");
        exit(0xE);
    }
    p = gsl_permutation_alloc(mtx->size1);
...
void vbg_vem(VBGMM *vbg, gmm *gm, data *dado, double alpha0, double beta0, double v0, gsl_vector *m0, gsl_matrix *W0)
{
    ...
    ///Initial parameters mean 'xbar' and covariance matrixes S (from EM) and priors pi
    for (i=0; i<K; i++)
    {
        S[i] = gsl_matrix_alloc(D,D);

        vset(pi,i,exp(gm->mix[i].prior));
        gsl_vector_set(Nk,i,N*vget(pi,i));
    ...
    for (t=0; t<VBGMMMAXITER; t++)
    {
        ...
        /*!Paralel computation of E - Eq 10.64*/
        arg1.K_ = arg2.K_ = K;
        arg1.D_ = arg2.D_ = D;
        arg1.s1_ = 0;
        arg2.s1_ = N>>1;
        ...
        {
            printf("Warning: Lower Bound decreased: %f\n",vget(vbg->L,t-1) - vget(vbg->L,t));
            cqueda++;
        }
        ...
        ///Stop if converges or decreased two times
        if((likIncr < THRES) || cqueda > 1)
            break;
